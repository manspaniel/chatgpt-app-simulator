...
export class ChatGPTAPI {
  ...
  constructor(opts: {
    ...
    const {
      sessionToken,
      markdown = true,
      apiBaseUrl = "/api/proxy/api",
      backendApiBaseUrl = "/api/proxy/backend-api",
  ...
  async sendMessage(
    ...
    opts: {
      ...
      onProgress?: (partialResponse: types.PromptResult) => void;
    } = {}
  ): Promise<types.PromptResult> {
    ...
    const body: types.ConversationJSONBody = {
      ...
      parent_message_id: opts.parentMessageId as string,
    };
    ...
    let response: types.PromptResult;
    ...
    return new Promise((resolve, reject) => {
      fetchSSE(url, {
        ...
        onmessage: (e) => {
          ...
          try {
            const parsedData: types.ConversationResponseEvent =
              JSON.parse(data);
            ...
            if (message) {
              const result: types.PromptResult = {
                messageId: message.id,
                messageText: message.content.parts[0],
  ...
  async refreshAccessToken(): Promise<string> {
    ...
    // await fetch(
    //   "/api/set-cookie?value=" + encodeURIComponent(this._sessionToken),
    //   {
    //     method: "GET",
    //     credentials: "same-origin",
    //   }
    ...
    try {
      const res = await fetch(
        "/api/proxy/api/auth/session?cookie=" +
          encodeURIComponent(this._sessionToken),
        {
          headers: {
            "user-agent": this._userAgent,
          },
